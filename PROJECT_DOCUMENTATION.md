...
- ✅ **Incremental / Delta Load** — SHA-256 row-level fingerprinting with insert/update/skip classification
- ✅ **Data Versioning & Snapshots** — point-in-time frozen copies of the curated dataset with field-level diffing
...
#### 4. **DELTA Layer** (Incremental Change Tracking) — NEW
Row-level change records generated by the incremental loader.

| Table | Purpose |
|-------|---------|
| `raw_delta_log` | Per-row insert/update/skip log for every delta pipeline run |

**`raw_delta_log` columns:**
- `run_id` — UUID linking back to the pipeline run
- `source_file` — originating Excel filename
- `table_name` — which RAW table was targeted
- `row_number` — position in source file
- `action` — `insert` | `update` | `skip`
- `checksum_old` / `checksum_new` — SHA-256 before/after
- `changed_fields` — JSONB `{field: {old, new}}` map (updates only)
- `logged_at` — timestamp

#### 5. **SNAPSHOT Layer** (Point-in-Time Versioning) — NEW
Frozen copies of the curated dataset for reproducible research.

| Table | Purpose |
|-------|---------|
| `curated_snapshots` | Snapshot metadata (label, counts, run linkage) |
| `snapshot_cases` | Denormalized frozen case rows (one per case per snapshot) |

**`curated_snapshots` columns:** `id, run_id, label, description, taken_at, case_count, doc_count, source_count, tag_count, is_auto`

**`snapshot_cases` columns:** all scalar case fields + `tag_values` JSONB + `row_checksum` SHA-256 for fast diffing

**Auto-Snapshot Behavior:**
- Both `POST /pipeline/load` and `POST /pipeline/load-delta` automatically take a snapshot on success
- Manual snapshots can be created via `POST /snapshots` (auth required)
- Each snapshot is an independent frozen copy — deleting it removes all `snapshot_cases` rows (CASCADE)
...
**Idempotency:** Full load truncates and reloads RAW tables, then re-transforms to CURATED. Automatically takes a snapshot on success.

#### Incremental / Delta Load — NEW

Processes only rows that have changed since the last run using SHA-256 checksums.

```bash
curl -X POST http://localhost:8000/pipeline/load-delta \
  -H "X-API-Key: dail-forge-secret-key-change-me"
```

**How it works:**
```python
For each row in each Excel file:
  new_checksum = SHA-256(sorted JSON of all content fields)
  
  if no existing row with same natural key:
    → INSERT, log action="insert"
  elif new_checksum != existing checksum:
    → UPDATE, log action="update", record changed_fields
  else:
    → SKIP, log action="skip"

if total_changes > 0:
  → run transform_all()   # re-sync CURATED layer
  → take_snapshot()       # auto-snapshot the new state
else:
  → skip transform (no changes detected)
```

**Returns:** Summary of `{inserted, updated, skipped}` per file plus `snapshot_id` if snapshot was taken.

#### Snapshot Management — NEW

Snapshots are frozen denormalized copies of the curated dataset taken automatically after every pipeline run. They can also be created manually:
...
# Diff snapshot against its predecessor
curl http://localhost:8000/snapshots/2/diff

# Diff against a specific earlier snapshot
curl http://localhost:8000/snapshots/5/diff?prev_id=2

# Manually create a labeled snapshot (auth required)
curl -X POST http://localhost:8000/snapshots \
  -H "X-API-Key: dail-forge-secret-key-change-me" \
  -H "Content-Type: application/json" \
  -d '{"label": "Pre-Q2 baseline", "description": "Snapshot before new data import"}'

# Delete a snapshot (auth required)
curl -X DELETE http://localhost:8000/snapshots/3 \
  -H "X-API-Key: dail-forge-secret-key-change-me"
```
...
```http
POST /pipeline/load
  Triggers full pipeline execution (truncate + reload)
  Returns: {run_id, status, snapshot_id}

POST /pipeline/load-delta          ← NEW
  Incremental load — only processes changed rows
  Returns: {run_id, delta: {inserted, updated, skipped, by_file}, snapshot_id}
...
#### Snapshots API — NEW
```http
GET /snapshots?limit=50
  Returns: List of snapshots ordered by taken_at desc
  
GET /snapshots/{id}
  Returns: Single snapshot metadata

GET /snapshots/{id}/diff?prev_id=N
  Returns: Field-level diff vs previous snapshot (or N if specified)
  Body: {
    added_count, removed_count, changed_count, unchanged_count,
    cases: [{case_pk, case_id, case_name, action, changed_fields}]
  }

POST /snapshots                    (auth required)
  Body: {label: string, description?: string}
  Returns: {id, label, taken_at, case_count, ...}

DELETE /snapshots/{id}             (auth required)
  Deletes snapshot + all frozen case rows (CASCADE)
...
**7. Pipeline** — NEW
- Full Reload card (triggers `POST /pipeline/load`)
- Incremental Delta Load card (triggers `POST /pipeline/load-delta`)
- Per-file insert/update/skip breakdown table
- Auto-links to Snapshots page when snapshot is created

**8. Snapshots** — NEW
- Live table of all snapshots (label, date, case count, auto/manual badge)
- Diff button per row — loads field-level diff inline
- Take Snapshot form (label + API key)
- Diff panel showing added/removed/changed counts + per-case changed fields
- Auto-refreshes list when tab is opened
...
│   ├── routes_research.py      # Public read endpoints (cases, export)
│   ├── routes_curation.py      # Restricted write endpoints (provenance)
│   ├── routes_pipeline.py      # Full load + delta load endpoints
│   ├── routes_snapshots.py     # Snapshot CRUD + diff endpoint  ← NEW
│   ├── routes_stats.py         # Dashboard statistics
│   └── routes_views.py         # Saved filter preset endpoints
│
├── db/                         # Database layer
│   ├── __init__.py
│   ├── models.py               # SQLAlchemy ORM models (RAW/CURATED/PROVENANCE/DELTA/SNAPSHOT)
...
│           ├── 0006_full_text_search.py
│           ├── 0007_geo_views_citations.py
│           ├── 0008_incremental_delta.py      ← NEW (row checksums + delta log)
│           └── 0009_snapshots.py              ← NEW (curated_snapshots + snapshot_cases)
│
├── pipeline/                   # Data ingestion & transformation
│   ├── __init__.py
│   ├── load_all.py            # Master pipeline orchestrator (full load)
│   ├── excel_loader.py        # Excel → RAW tables (schema-aware)
│   ├── delta_loader.py        # Incremental / delta loader  ← NEW
│   ├── snapshot.py            # take_snapshot() + diff_snapshots()  ← NEW
│   ├── transform.py           # RAW → CURATED (stub synthesis)
│   ├── validate.py            # Post-load data validation
│   ├── column_map.py          # Fuzzy column name mapping
│   └── geo_map.py             # State/circuit geographic helpers
│
...
│   └── index.html             # Dashboard + Terminal + Pipeline + Snapshots + Curation UI
│
...
- [ ] Delta load (`POST /pipeline/load-delta`) returns insert/update/skip counts
- [ ] `raw_delta_log` rows written for each processed file
- [ ] Re-running delta load on unchanged data returns all `skip`
- [ ] Auto-snapshot created after successful pipeline run
- [ ] `GET /snapshots` returns snapshot list in descending order
- [ ] `GET /snapshots/{id}/diff` returns field-level diff
- [ ] Deleting a snapshot cascades to `snapshot_cases`
- [ ] Pipeline and Snapshots tabs visible and functional in frontend
...
### Database Migrations
All migrations tracked in `db/migrations/versions/` and applied automatically by `alembic upgrade head` on startup:
...
6. `0006_full_text_search.py` - Full-text search indexes
...
- [x] Incremental / delta load with SHA-256 row checksums
- [x] Data versioning with point-in-time snapshots
- [x] Field-level snapshot diffing (added/removed/changed)
- [x] Auto-snapshot on every pipeline run
- [x] Saved views (filter presets)
- [x] Full-text search indexes
- [x] Geographic state/circuit helpers
...
2. **Layered Architecture** - RAW → CURATED → PROVENANCE → DELTA → SNAPSHOT separation
3. **Idempotent Pipeline** - Safe to re-run without duplication
4. **Incremental by Default** - Use delta load to minimize database writes for unchanged data
5. **Schema Flexibility** - Adapt to evolving data structures
6. **Research-Grade** - Clean, documented, reproducible
7. **Snapshot Integrity** - Each snapshot is an independent, immutable frozen copy
...
- ✅ Five-layer database architecture (RAW/CURATED/PROVENANCE/DELTA/SNAPSHOT)
...
### v0.2.0 (February 2026) - Incremental Load & Versioning
- ✅ Incremental / delta load (`POST /pipeline/load-delta`) with SHA-256 per-row checksums
- ✅ `raw_delta_log` table — per-row insert/update/skip audit log
- ✅ Data versioning with `curated_snapshots` + `snapshot_cases` tables
- ✅ Field-level snapshot diffing via `GET /snapshots/{id}/diff`
- ✅ Auto-snapshot on every pipeline run (full and delta)
- ✅ Pipeline and Snapshots pages in frontend UI
